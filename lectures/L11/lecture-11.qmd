...
4.  **Check assumptions**: Remember - it's about the residuals! If assumptions fail, try transforming and refit the model. If assumptions still fail, consider another model.

5.  **Assess model fit**: Hypothesis test, significance, F-statistic, p-value. R^2^, how much model variation was explained by the model. **Parsimony**
  
6.  **Interpret output**: 'For every one-unit increase in x, y increases by $\beta_1$ units...' and any additional research/insight. **different for MLR**
...
## Parsimony in regression modeling
...
An extension of simple linear regression to include **more than one** predictor variable: 
...
## Visualisation is not easy: are the plots useful?
...
## Partial regression coefficients: Air quality example
...
High (but not perfect) collinearity (e.g. strong/very strong $r$) does not break the assumption but can lead to unstable estimates and large standard errors.

> The largest correlation between the predictors is between Temp and Wind ($r$ = -0.5). This is not a problem.

## Let's check our assumptions for the Air quality model
...
Some evidence of non-linearity in the diagnostic plots. We need to transform and re-check assumptions.
...
The assumptions are met, let's move onto interpretation. 
...
- The MLR model explains 66% of variation in log(Ozone) (adjusted R^2^ = 0.66), whereas the SLR explains 48% of variation in log(Ozone) (multiple R^2^ = 0.48).
...
## Interpreting coefficients - backtransformed approximation
...
- The MLR model explains 66% of variation in log(Ozone) (**adjusted** R^2^ = 0.66)
...
- Solar radiation, wind speed and temperature are **significant predictors** of Ozone concentration (**p < 0.001**) with the model accounting for **66% of the variation** in log(Ozone). The model explained more variation than a single-predictor model and was found to be significantly better than the null model.
