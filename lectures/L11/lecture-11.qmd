...
e.g. if a simple (one variable) model and a complex (many variables) model predict similarly well, the simple model is preferred.
...
- However, the model may not be able to **generalise** to new data. The model is **overfitted**, i.e. too complex.
...
- *C*ollinearity: there is no perfect linearity between predictors
...
```{r}
exp(0.51) #backtransform with e^0.51
```
...
Would mention the following in a report (directly after figures/evidence):
...
> There were non-linear relationships between `Ozone` (the response) and `Temp`, `Wind` and `Solar.R` (the predictors), hence a natural log transoformation was applied to `Ozone`.
...
- Fit multiple dimensions (hyperplane)
