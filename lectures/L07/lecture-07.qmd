...
**Relax** assumptions about the distribution of the data. 
...
2. If the normality assumption is violated, we first try transform the data and check for normality again.  
3. If the data is stubborn and not transforming well, **Non-parametric tests** are a good next step.
4. Often used when sample sizes are small (<15ish) and can't easily transform the data.
...
The Wilcoxon signed-rank test is a non-parametric test used to assess whether a single sample differs from zero, or whether paired samples differ.
...
- Independent observations (or independent pairs)
- Data represent differences from a value (normally zero) (or between pairs)
- Differences are fairly balanced around zero

. . .

Basically, used in same situations as the one-sample or paired $t$-test, but when the data is not normally distributed but **still reasonably balanced**.
...
> Is there a significant increase in weight gain after the diet - indicates a one tailed test?

. . . 

Sometimes we state the hypothesis in terms of the median as it is a robust estimator of the centre of the distribution:

- $H_0$: the median difference between the paired samples is equal to zero
- $H_1$: the median difference between the paired samples is greater than zero 

. . .

What we really mean is:

- $H_0$: the distribution of the difference between the paired samples is centered at zero
- $H_1$: the distribution of the difference between the paired samples is centered at a value greater than zero 
...
With so few data points (small sample), we may want to use a formal test to check for normality.
...
- Results indicate that the data significantly deviates from normality (W = 0.63, p < 0.05). 
- because it is a small sample size and the data is not normally distributed, we decide to use the Wilcoxon signed-rank test instead of transforming the data.
...
- $H_0$: the distribution of smaller lizard beetle consumption is centred at 100 beetles per day
- $H_1$: the distribution of smaller lizard beetle consumption is not centred at 100 beetles per day
...
::: callout-important
We not easily able to make a conclusion about effect size (confidence intervals and mean) from non-parametric tests as the information is lost when the data is transformed into ranks.
:::
...
More accurately, we are testing for a difference in centre of the *distribution* of the two groups and the median is a robust estimator of this.
...
Data does not meet the normality assumption? Compare the shapes of the distributions
...
- The conventional wisdom is to **transform the data** and check for normality if the assumption is not met. If the data is *still* not normal, then use non-parametric tests (after considering the implications on interpretation).
  - Or, use simulations based approaches (next week!).
...
::: incremental
- So far, **all** of our techniques have been aimed at comparing **means/medians/central mass** of continuous variables.
